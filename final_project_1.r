...
generate_acc <- function(est_df, models, acc_method = "mean") {
  # calculate acc
  ...
  } else if (acc_method == "estimated_variance") {
    acc_dt <- est_dt[,
                     .(acc_coef_beta0 = sqrt(sum((coef_beta0 - mean(coef_beta0))^2) / (.N - 1)),
                       acc_coef_beta1 = sqrt(sum((coef_beta1 - mean(coef_beta1))^2) / (.N - 1)),
                       acc_coef_beta2 = sqrt(sum((coef_beta2 - mean(coef_beta2))^2) / (.N - 1)),
                       acc_coef_beta3 = sqrt(sum((coef_beta3 - mean(coef_beta3))^2) / (.N - 1)),
                       acc_coef_beta4 = sqrt(sum((coef_beta4 - mean(coef_beta4))^2) / (.N - 1)),
                       acc_sd_beta0 = sqrt(sum((coef_sd_beta0 - mean(coef_sd_beta0))^2) / (.N - 1)),
                       acc_sd_beta1 = sqrt(sum((coef_sd_beta1 - mean(coef_sd_beta1))^2) / (.N - 1)),
                       acc_sd_beta2 = sqrt(sum((coef_sd_beta2 - mean(coef_sd_beta2))^2) / (.N - 1)),
                       acc_sd_beta3 = sqrt(sum((coef_sd_beta3 - mean(coef_sd_beta3))^2) / (.N - 1)),
                       acc_sd_beta4 = sqrt(sum((coef_sd_beta4 - mean(coef_sd_beta4))^2) / (.N - 1))),
                     by = .(X_col_level, X_R2, X_sample_size, X_model)]
  } else if (acc_method == "charlotte") {
    acc_dt <- est_dt[,
                     .(acc_coef_beta0 = abs(coef_beta0 - models[[X_model]][1]),
                       acc_coef_beta1 = abs(coef_beta1 - models[[X_model]][2]),
                       acc_coef_beta2 = abs(coef_beta2 - models[[X_model]][3]),
...
acc_data <- generate_acc(ds$est_df, model_factor, acc_method = "mean")
...
R2_facet_labeller <- function(value) {
  return(data.frame("R2 = 0.1", "R2 = 0.25", "R2 = 0.5", "R2 = 0.75"))
}
...
ggplot(mean_acc_data, aes(x = X_sample_size, y = mean_acc_coef_beta1, 
  ...
  scale_x_discrete(name = expression(paste("Sample Size ", N))) +
  scale_color_discrete(name = "Collinearity Level") + 
  theme_publish(base_size = 15, base_linewidth = 1)

# plot mean acc coef beta2
ggplot(mean_acc_data, aes(x = X_sample_size, y = mean_acc_coef_beta2, 
                          color = X_col_level)) +
  geom_point() +
  geom_line(mapping = aes(group = X_col_level)) + 
  facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
...
ggplot(mean_acc_data, aes(x = X_sample_size, y = mean_acc_coef_beta4, 
  ...
  geom_line(mapping = aes(group = X_col_level)) + 
  facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
  scale_y_continuous(name = expression(paste("Estimated ", hat(beta)[4], " SD"))) +
  scale_x_discrete(name = expression(paste("Sample Size ", N))) +
  scale_color_discrete(name = "Collinearity Level") + 
  theme_publish(base_size = 15, base_linewidth = 1)

# # plot mean acc coef beta0
# ggplot(mean_acc_data, aes(x = X_sample_size, y = mean_acc_coef_beta0, 
#                           color = X_col_level)) +
#   geom_point() +
#   geom_line(mapping = aes(group = X_col_level)) + 
#   facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
#   scale_y_continuous(name = "Estimated Beta0 SD") +
...
#   geom_line(mapping = aes(group = X_col_level)) + 
#   facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
#   scale_y_continuous(name = "Estimated SD for Beta1 SD") +
#   scale_x_discrete(name = "Sample Size") +
#   scale_color_discrete(name = "Collinearity Level") + 
#   theme_publish()
#   
