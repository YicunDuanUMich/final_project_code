...
for (col_strength in col_strengths) {
  a <- col_strength
  b <- abs(col_strength) / 3
  col_level_factor[[as.character(col_strength)]] <- matrix(c(1.0, a, b, 0.0,
                                                             a, 1.0, b, 0.0,
                                                             b, b, 1.0, 0.0,
...
sapply(col_level_factor, function(x) {
  return(calculate_condition_number(generate_mvnorm_samples(x, 100)))
})
...
for (model_angle in model_angles) {
  model_factor[[as.character(model_angle)]] <- c(2, 
                                                 cos(model_angle), sin(model_angle),
                                                 0.0, 0.25)
}
...
calculate_sd <- function(est_df) {
  ...
  est_dt <- est_dt[,
                   .(est_sd_beta0 = sqrt(sum((coef_beta0 - mean(coef_beta0))^2) / (.N - 1)),
                     ...
                     est_sd_beta4 = sqrt(sum((coef_beta4 - mean(coef_beta4))^2) / (.N - 1))),
...
ggplot(est_sd_dt, aes(x = X_col_level, y = est_sd_beta3, 
                      color = X_model)) +
  geom_point() +
  geom_line(mapping = aes(group = X_model)) + 
  facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
  scale_y_continuous(name = "Beta3 SD") +
  scale_x_discrete(name = "Collinearity Level") +
  scale_color_discrete(name = "Angle of (Beta1, Beta2)") + 
  theme_publish()

# plot est sd of beta4
ggplot(est_sd_dt, aes(x = X_col_level, y = est_sd_beta4, 
                      color = X_model)) +
  geom_point() +
  geom_line(mapping = aes(group = X_model)) + 
  facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
  scale_y_continuous(name = "Beta4 SD") +
  scale_x_discrete(name = "Collinearity Level") +
  scale_color_discrete(name = "Angle of (Beta1, Beta2)") + 
  theme_publish()

# plot est sd of beta0
ggplot(est_sd_dt, aes(x = X_col_level, y = est_sd_beta0, 
                      color = X_model)) +
  geom_point() +
  geom_line(mapping = aes(group = X_model)) + 
  facet_grid(cols = vars(X_R2), labeller = labeller(X_R2 = R2_facet_labeller)) +
  scale_y_continuous(name = "Beta0 SD") +
  scale_x_discrete(name = "Collinearity Level") +
  scale_color_discrete(name = "Angle of (Beta1, Beta2)") + 
  theme_publish()
